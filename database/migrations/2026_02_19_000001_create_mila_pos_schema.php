...
/**
 * Migracion unica que reproduce, statement por statement, los bloques 1-14
 * del esquema mila_pos (database/boss_pos_schema.sql): catalogos, seguridad,
 * productos/inventario, cajas/folios, promociones, ventas, compras,
 * devoluciones, traspasos, CxC, conteos/gastos/configuracion/auditoria,
 * triggers, procedimiento de folios y vistas. Se ejecuta tal cual contra la
 * conexion configurada (mila_pos) via DB::unprepared, sin depender de
 * DELIMITER (eso es solo un artefacto del cliente `mysql`, no de PDO).
 *
 * Bloque 15 (datos semilla) vive en database/seeders/. Bloque 16 (usuarios de
 * BD / GRANTs) es [OPS] y no se ejecuta desde la aplicacion.
 */
return new class extends Migration
{
    public function up(): void
    {
        // db:wipe (usado por migrate:fresh y por RefreshDatabase en tests) solo
        // dropea tablas y vistas: los triggers y procedimientos de una corrida
        // anterior sobreviven y chocan con el CREATE. Se limpian a mano antes
        // de crear, para que esta migracion sea segura de re-ejecutar.
        foreach ($this->dropRoutineStatements() as $sql) {
            DB::unprepared($sql);
        }
    ...
    {
        foreach ($this->dropStatements() as $sql) {
            DB::unprepared($sql);
        }
    }
     ...
     */
    private function dropRoutineStatements(): array
    {
        return [
            'DROP TRIGGER IF EXISTS trg_productos_au_historial',
            'DROP TRIGGER IF EXISTS trg_ventas_au_auditoria',
            'DROP TRIGGER IF EXISTS trg_movinv_bu_bloquear',
            'DROP TRIGGER IF EXISTS trg_movinv_bd_bloquear',
            'DROP PROCEDURE IF EXISTS sp_siguiente_folio',
        ];
    }
...
SQL,
    ...
    {
        return [
            'DROP VIEW IF EXISTS v_arqueo_corte',
            'DROP VIEW IF EXISTS v_stock_bajo',
            'DROP VIEW IF EXISTS v_utilidad_venta',
            'DROP PROCEDURE IF EXISTS sp_siguiente_folio',
            'DROP TRIGGER IF EXISTS trg_movinv_bd_bloquear',
            'DROP TRIGGER IF EXISTS trg_movinv_bu_bloquear',
            'DROP TRIGGER IF EXISTS trg_ventas_au_auditoria',
            ...
            'DROP TABLE IF EXISTS devolucion_pagos',
            'DROP TABLE IF EXISTS detalle_devolucion',
            'DROP TABLE IF EXISTS devoluciones',
            'DROP TABLE IF EXISTS detalle_compra',
            'DROP TABLE IF EXISTS compras',
            'DROP TABLE IF EXISTS corte_caja_metodos',
            'DROP TABLE IF EXISTS venta_pagos',
            'DROP TABLE IF EXISTS detalle_venta',
            'DROP TABLE IF EXISTS ventas',
            'DROP TABLE IF EXISTS promocion_alcance',
